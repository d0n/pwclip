...
def run_operation(jboss_config, operation, fail_on_error=True, retries=1):
    ...
    '''
    if '__dbg__' in jboss_config.keys() and jboss_config['__dbg__']:
        print('jboss_config: %s, operation: %s, fail_on_error: %s, retries: %s'%(jboss_config, operation, fail_on_error, retries))
...
def __call_cli(jboss_config, command, retries=1):
    ...
    cli_command_result = run_all(cli_script)
...
def __process_tokens_internal(tokens, start_at=0):
    ...
    while token_no < len(tokens):
        ...
        if __is_quoted_string(token):
            ...
            is_assignment = True
        #else:
        #    print(current_key, token)
        #    #aise CommandExecutionError('Unknown token: '+token)
