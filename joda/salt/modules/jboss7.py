...
from .jboss7_cli import run_operation, run_command
...
def status(jboss_config):
    ...
    return run_operation(jboss_config, operation, fail_on_error=False, retries=0)
...
def stop_server(jboss_config):
    ...
    operation = ':shutdown'
    shutdown_result = run_operation(jboss_config, operation, fail_on_error=False)
...
def reload_(jboss_config):
    ...
    operation = ':reload'
    reload_result = run_operation(jboss_config, operation, fail_on_error=False)
...
def create_datasource(jboss_config, name, datasource_properties):
    ...
    operation = '/subsystem=datasources/data-source="{name}":add({properties})'.format(
                name=name,
                properties=__get_properties_assignment_string(dict(datasource_properties), ds_resource_description))
    return run_operation(jboss_config, operation, fail_on_error=False)
...
def list_datasources(jboss_config):
    '''
    result   = {'data-source': {'AegirMigraeneDS': None, 'SmartdriveIdPDS': None, 'EcosMandantDS': None, 'EcosMigraeneDS': None, 'ConfigDS': None, 'MigraeneDS': None, 'ThorMigraeneDS': None}, 'jdbc-driver': {'postgresql': None, 'mysql': None, 'oracle': None, 'sybase': None, 'h2': None}, 'xa-data-source': {'EcosDS': None, 'StatsCollectorDS': None, 'ScopsDS': None, 'QuaggaDS': None, 'IsisDS': None}}
    '''
    out = run_operation(jboss_config, '/subsystem=datasources:read-resource')
    if out and 'result' in out.keys() and out['result']:
        out = out['result']
    if out and 'data-source' in out.keys() and out['data-source']:
        return [d.split('=')[0] for d in out['data-source'] if d]
...
def __get_datasource_resource_description(jboss_config, name):
    ...
    operation_result = run_operation(jboss_config, operation)
...
def create_simple_binding(jboss_config, binding_name, value):
    ...
    return run_operation(jboss_config, operation)
...
def deploy(jboss_config, source_file):
    ...
    return run_command(jboss_config, command, fail_on_error=False)
...
def list_deployments(jboss_config):
    ...
    command_result = run_command(jboss_config, 'deploy')
    deployments = []
    if len(command_result['stdout']) > 0:
        deployments = command_result['stdout']
...
def undeploy(jboss_config, deployment):
    ...
    return run_command(jboss_config, command)
